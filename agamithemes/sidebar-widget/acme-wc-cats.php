 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'agami_shop_wc_feature_type_default' ) ) :
	function agami_shop_wc_feature_type_default( $agami_shop_featured_cats, $term_per_slide, $agami_shop_img_size, $number_of_product_text ){
		$i = 1;
		$j = 1;
		$total_items = count( $agami_shop_featured_cats );
		$remaining_items = $total_items;

		$current_slide_number = 1;
		$agami_shop_slider_display_options = $term_per_slide;

		$total_posts = $agami_shop_slider_display_options;
		if( $agami_shop_slider_display_options > $remaining_items ){
			$total_posts = $remaining_items;
		}

		$fixed ='';
		if( $remaining_items < $agami_shop_slider_display_options || $agami_shop_slider_display_options < 3 ){
		...
		foreach ( $agami_shop_featured_cats as $term_id => $value ) {
			...
			if ( $term && ! is_wp_error( $term ) ) {
				...
				$image_url = wp_get_attachment_image_src( $thumbnail_id, $agami_shop_img_size );
				if ( !$image_url ) {
					$image_url[0] =  get_template_directory_uri() . '/assets/img/default-image.jpg';
				}
				if( $j > $agami_shop_slider_display_options ){
					if( $agami_shop_slider_display_options == 1 || $j % $agami_shop_slider_display_options == 1 ){
						$current_slide_number = $current_slide_number + 1;
						$remaining_items = $remaining_items - $agami_shop_slider_display_options;
						if( $agami_shop_slider_display_options > $remaining_items ){
							$total_posts = $remaining_items;
						}
						$fixed ='';
						if( $remaining_items < $agami_shop_slider_display_options || $agami_shop_slider_display_options < 3 )
							if( $remaining_items < $agami_shop_slider_display_options ){
								$fixed = 'fix remain-'.$remaining_items;
							}
						$i = 1;
...
if ( ! function_exists( 'agami_shop_wc_feature_type_two' ) ) :
	function agami_shop_wc_feature_type_two( $agami_shop_featured_cats, $term_per_slide, $agami_shop_img_size, $number_of_product_text ){
		$i = 1;
		$j = 1;
		$total_items = count( $agami_shop_featured_cats );
		$remaining_items = $total_items;
 ...
 * @since 1.0.0
 */
if ( ! class_exists( 'agami_shop_Wc_Feature_Cats' ) ) {
    /**
     * Class for adding widget
     *
     * @package Agami Themes
     * @subpackage agami_shop_Wc_Feature_Cats
     * @since 1.0.0
     */
    class agami_shop_Wc_Feature_Cats extends WP_Widget {
        ...
        function __construct() {
            ...
            /*Base ID of your widget*/
                ...
                esc_html__('AT WooCommerce Categories', 'agami-shop'),
                /*Widget description*/
                array( 'description' => esc_html__( 'Show WooCommerce Categories Beautifully', 'agami-shop' ), )
            );
        }
        ...
        public function form( $instance ) {
	        ...
	        $agami_shop_img_size = $instance['agami_shop_img_size'];
	        ...
	        if( !empty( $woocommerce_categories_obj ) ){
	            ...
	            esc_html_e('No Categories found', 'agami-shop');
            }
	        ?>
            </div>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'layout_type' ) ); ?>">
			        <?php esc_html_e( 'Select Layout', 'agami-shop' ); ?>
                </label>
                <select class="widefat at-display-select" id="<?php echo esc_attr( $this->get_field_id( 'layout_type' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'layout_type' ) ); ?>" >
			        <?php
			        $agami_shop_wc_cat_layout_types = agami_shop_wc_cat_layout_type();
			        foreach ( $agami_shop_wc_cat_layout_types as $key => $value ){
				        ?>
            ...
            <p>
                ...
                <label for="<?php echo esc_attr( $this->get_field_id( 'enable_slider_mode' ) ); ?>"><?php esc_html_e( 'Enable Slider Mode', 'agami-shop' ); ?></label>
            </p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'term_per_slide' ) ); ?>"><?php esc_html_e( 'Cat Per Slide', 'agami-shop' ); ?></label>
                <select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'term_per_slide' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'term_per_slide' ) ); ?>" >
			        <?php
			        $agami_shop_widget_term_per_slides = agami_shop_widget_term_per_slide();
			        foreach ( $agami_shop_widget_term_per_slides as $key => $value ){
				        ?>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'number_of_product_text' ) ); ?>">
			        <?php esc_html_e( 'Number of product text', 'agami-shop' ); ?>
                </label>
            ...
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'agami_shop_img_size' ) ); ?>">
			        <?php esc_html_e( 'Image Size', 'agami-shop' ); ?>
                </label>
        ...
        public function update( $new_instance, $old_instance ) {
	        ...
	        $agami_shop_wc_cat_layout_types = agami_shop_wc_cat_layout_type();
	        $instance[ 'layout_type' ] = agami_shop_sanitize_choice_options( $new_instance[ 'layout_type' ], $agami_shop_wc_cat_layout_types, 2 );
        ...
        public function widget($args, $instance) {
	        ...
	        if(!empty( $agami_shop_featured_cats ) ){
                ...
                <div class="wc-cat-feature <?php echo esc_attr( $class ); ?> layout-<?php echo esc_attr( $layout_type );?>" data-column="1">
                    <?php
                    if( 2 == $layout_type ){
	                    agami_shop_wc_feature_type_two( $agami_shop_featured_cats, $term_per_slide, $agami_shop_img_size, $number_of_product_text );
                    }
                    else{
	                    agami_shop_wc_feature_type_default( $agami_shop_featured_cats, $term_per_slide, $agami_shop_img_size, $number_of_product_text );
                    }
                    ?>
                </div><!--wc-cat-feature-->
		        <?php
    ...
    } // Class agami_shop_Wc_Feature_Cats ends here
}